...
function LocationDialog({
  ...
  const [open, setOpen] = useState(false);
  const [nombre, setNombre] = useState(
    location?.nombre === "___EMPTY___" ? "" : (location?.nombre ?? ""),
  );
...
function UbicacionesPage() {
  ...
  const agrupado = db.locations.reduce(
    (acc, loc) => {
            ...
            {Object.entries(agrupado).map(([sedeName, pisos]) => (
              ...
              >
                <AccordionTrigger className="px-6 py-4 hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-2 text-xl font-bold">{sedeName}</div>
                </AccordionTrigger>
                              ...
                              <Table>
                                <TableHeader>
                                  <TableRow>
                                    ...
                                    {canEdit("ubicaciones") && (
                                      <TableHead className="text-right w-40 pr-4">
                                        Acciones
                                      </TableHead>
                                    )}
                                  </TableRow>
                                ...
                                <TableBody>
                                  {locs.length === 0 ? (
                                    ...
                                    locs.map((l) => {
                                      const eqCount = db.equipment.filter(
                                        (e) => e.ubicacionId === l.id,
                                      ).length;
                                      return (
                                        <TableRow key={l.id}>
